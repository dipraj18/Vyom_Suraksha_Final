...
# Auto-initialize daemon if imported directly (e.g., via Gunicorn)
def init_daemon_if_needed():
    global orchestrator, policy_engine, trust_core, risk_engine, secret_guard, audit, stealth, state_controller, canary
    if orchestrator is not None:
        return

    logging.info("[WSGI] Auto-initializing Vyom Suraksha daemon components...")
    
    # Import necessary modules inside to avoid circular imports
    import threading
    ...
    for log_dir in ["logs/alerts", "logs/audit", "logs/backup", "logs/beacon", "logs/decisions", "logs/remote_storage", "logs/runtime", ".secure_keys"]:
    ...
    if not sec_config:
        sec_config = {
            "risk_thresholds": {"alert": 30, "containment": 60, "lockdown": 90}
        }
    ...
    policy_engine = PolicyEngine(risk_engine, state_controller, decision_engine)
    ...
    try:
        ...
        if not os.path.exists(key_path):
            logging.info(f"Backup encryption key not found. Generating at: {key_path}")
        ...
        logging.critical(f"Backup subsystem disabled: {e}")
    ...
    audit.log_event("SYSTEM_START", {"mode": env_config.get("mode", "production")})

    def daemon_loop():
        cycle = 0
        prev_state_value = None
        current_sec_config = sec_config
            ...
            while True:
                ...
                try:
                    ...
                    if updated_sec_config and "risk_thresholds" in updated_sec_config:
                        current_sec_config = updated_sec_config
                    ...
                    policy_result = policy_engine.evaluate(
                        ...
                        thresholds=current_sec_config["risk_thresholds"]
                    )
                    ...
                    if prev_state_value != current_state.value:
                        logging.warning(f"[WSGI Daemon] State changed → {current_state.value} (Risk: {risk_score}, Trust: {trust_score})")
                    ...
                    logging.error(f"[WSGI Daemon] Runtime loop error: {ex}")
                time.sleep(5)
        except Exception as e:
            logging.critical(f"[WSGI Daemon] Fatal loop exception: {e}")
...
@app.before_request
def auto_init_daemon():
    init_daemon_if_needed()
